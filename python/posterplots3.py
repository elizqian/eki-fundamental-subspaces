...
fig, axs = plt.subplots(2,2, sharex='col', sharey='row', figsize=(24,20))
...
for i in range(2): # col
    for j in range(2): # row
        for k in [1,2,0]: # linetype
            ...
            if k == 0:
                scl = np.max(y[1,:])
            ln = axs[i,j].loglog(x,y,alpha=0.3,color=colors[k],linestyle=styles[k],label=lbls[i][k],linewidth=8)
            prox = Line2D([0], [0], color=colors[k],linestyle=styles[k], label=lbls[i][k],linewidth=8)
            if j == 0:
                lines[i].append(prox)
        sqrt = axs[i,j].loglog(xx,scl/np.sqrt(xx),color="#555555",alpha=0.8,linewidth=8)
        style_axes(axs[i,j])

axs[1,0].set_xlabel("Iteration number $i$",fontsize=60,labelpad=2)
axs[1,1].set_xlabel("Iteration number $i$",fontsize=60,labelpad=2)
axs[0,0].set_title("Deterministic EKI",pad=10,fontsize=72)
axs[0,1].set_title("Stochastic EKI",fontsize=72,pad=0)

axs[0,0].set_ylabel("Data misfit",fontsize=66)
axs[1,0].set_ylabel("Least squares residual",fontsize=66)
...
axs[0,0].legend(lines_all,lbls_all,loc='lower left',bbox_to_anchor=(-0.1,-0.42),ncols=3,frameon=False,fontsize=54,handletextpad=0.2,columnspacing = 1)

# Legend for a 1/sqrt(i) rate line
proxy_line = Line2D([0], [0], color='#555555', alpha=0.8, label="$1/\\sqrt{i}$ rate",linewidth=8)
fig.legend(handles=[proxy_line], loc='center left', bbox_to_anchor=(0.75, 0.51), frameon=False,handletextpad=0.2,fontsize=54)
...
plt.subplots_adjust(top=0.95,right=0.97,left=0.12,bottom=0.08,hspace=0.33,wspace=0.1)
# plt.tight_layout()
fig.savefig("posterResults3.png",dpi=300)
plt.close()