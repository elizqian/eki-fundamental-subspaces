...
with open("bigrun.pkl","rb") as f:
    eki_runs = pickle.load(f)

det        = eki_runs[0]
stochsmall = eki_runs[1]
stochlarge = eki_runs[2]

n = 500
d = 1000
J = 2000

maxiter, d, Jsmall = det.vv.shape
maxiter -= 1
J = stochlarge.vv.shape[2]
...
for i in range(2): # row
    for j in range(3): # column
        for k in [2,1,0]: # linetype
            ...
            if k == 0:
                scl = np.max(y[1,:])
            ln = axs[i,j].loglog(x,y[:,:15],alpha=0.3,color=colors[k],linestyle=styles[k],label=lbls[i][k])
