...
# plot RMSEs for referee response
lines = [[],[]]
fig, axs = plt.subplots(2, 4, sharex='col', sharey='row', figsize=(5.5,4))
...
for i in range(2): # row
    for j in range(4): # column
        for k in [2,1,0]: # linetype
            y = np.sqrt(cols[j].errs[:,i*3+k,1])
            if i == 0:
                scl = 50
            else:
                scl = 5
            ln = axs[i,j].loglog(x,y,color=colors[k],linestyle=styles[k],label=lbls[i][k])
