...
fig, axs = plt.subplots(3,2, sharex='col', sharey='row', figsize=(24,24))
...
axs[2,0].set_xlabel("Iteration number $i$",fontsize=60,labelpad=2)
axs[2,1].set_xlabel("Iteration number $i$",fontsize=60,labelpad=2)
axs[0,0].set_title("Data misfit",fontsize=72,pad=150)
axs[0,1].set_title("Least squares residual",fontsize=72,pad=150)

axs[0,0].set_ylabel("Deterministic EKI",fontsize=60)
axs[1,0].set_ylabel("Stochastic EKI \n (large ensemble)",fontsize=60)
axs[2,0].set_ylabel("Stochastic EKI \n (small ensemble)",fontsize=60)
...
axs[0,0].legend([lines[0][2],lines[0][0],lines[0][1]],[lbls[0][0],lbls[0][1],lbls[0][2]],frameon=False,loc='upper left',fontsize=54,handletextpad=0.1,bbox_to_anchor=(-0.3,1.4),ncols=3,columnspacing=0.1,handlelength=1.25)

axs[0,1].legend([lines[1][2],lines[1][0],lines[1][1]],[lbls[1][0],lbls[1][1],lbls[1][2]],frameon=False,loc='upper left',fontsize=54,handletextpad=0.1,bbox_to_anchor=(-0.1,1.4),ncols=3,columnspacing=0.1,handlelength=1.25)

# # Legend for a 1/sqrt(i) rate line
proxy_line = Line2D([0], [0], color='#555555', alpha=0.8,linewidth=8, label="$1/\\sqrt{i}$ rate")
fig.legend(handles=[proxy_line], loc='center left', bbox_to_anchor=(0.7, 0.8), frameon=False,handletextpad=0.2,fontsize=54)
fig.legend(handles=[proxy_line], loc='center left', bbox_to_anchor=(0.27, 0.8), frameon=False,handletextpad=0.2,fontsize=54)

plt.subplots_adjust(top=0.85,right=0.93,left=0.14,bottom=0.08,hspace=0.1,wspace=0.1)
fig.savefig("posterResults.pdf")
plt.close()
...
axs[0,0].set_title("Data misfit",pad=0,fontproperties=latoLabel)
axs[0,1].set_title("Least squares residual",fontsize=72,pad=150)
